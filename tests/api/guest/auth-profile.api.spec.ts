...
test.describe('Kiểm thử API Profile (Supabase Database & Storage) sử dụng Playwright Auth State', () => {
  let accessToken = '';

  test.beforeAll(() => {
    accessToken = getAccessToken();
    if (accessToken) {
      console.log('[Profile API Test] Đã tìm thấy Access Token từ playwright/.auth/user.json');
    } else {
      console.warn('[Profile API Test] Không tìm thấy Access Token. Một số test case yêu cầu xác thực sẽ bị skip.');
    }
  });
